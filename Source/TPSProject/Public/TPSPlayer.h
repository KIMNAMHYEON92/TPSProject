...
public:
	...
	// 걷기 IA 필드 - 기본은 달리기, 누르는 동안 걷기
	UPROPERTY(EditDefaultsOnly, Category = Input)
	class UInputAction* ia_Walk;
	
	// 현재 유탄총을 사용중인지 여부 (true : 유탄총 / false : 스나이퍼건)
	...
	// 스나이퍼 줌 UI 위젯 팩토리 (WBP_SniperUI를 에디터에서 할당)
	...
	// 걷기 입력 함수 선언
	void InputWalk(const struct FInputActionValue& inputValue);
	...
	// 스나이퍼건 줌 입력 함수 선언
	void SniperZoom(const struct FInputActionValue& inputValue);
};
